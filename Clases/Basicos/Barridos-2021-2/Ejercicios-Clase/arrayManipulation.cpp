...
void solve(){
    int n, q, pos; cin>>n>>q>>pos;
    vector<int> arrMarquitas(n+1,0);
    //vector<lli> PrefxSum(n+1,0);
    ...
    for(int i = 1; i <= n ; i++ ){
        arrMarquitas[i] += arrMarquitas[i-1];
        //cout<<arrMarquitas[i-1]<<" ";
    }
    cout<<arrMarquitas[pos]<<endl;
}
