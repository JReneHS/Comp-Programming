...
const int MAX_TEMP = 2e5 + 2;

void solve(){
    int n,k,q,l,r; cin>>n>>k>>q;
    vli marquitas(MAX_TEMP,0), prefix_Sum_Upper_K(MAX_TEMP,0);
    ...
    foria(i, 1, MAX_TEMP){
        marquitas[i] += marquitas[i-1];
        prefix_Sum_Upper_K[i] = (marquitas[i] >= k ? 1 : 0);
        prefix_Sum_Upper_K[i] += prefix_Sum_Upper_K[i-1];
    }

    while(q--) {
        cin>>l>>r;
        cout<<prefix_Sum_Upper_K[r] - prefix_Sum_Upper_K[l-1]<<endl;
    }
}
